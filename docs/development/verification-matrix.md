...
| Milestone 7.2 live acceptance matrix | 12 of 15 mandatory rows passed on official OpenGOAL v0.3.5. Fresh/repeated saves, game and ordered dual restarts, save switching, copied-slot and vanilla-save rejection, no-save clearing, distinct New Game and overwrite identity, harmless-command replay/no-op behavior, and title-menu safety passed. Clean and unclean client-only replacement failed to reattach to the still-running game, and locked bank files crashed `gk` before a graceful native failure diagnostic. | This is real native-save acceptance with disposable saves and AP state. Rows 3, 4, and 13 remain mandatory failures, so Milestones 7 and 7.2 remain pending. See [milestone-7.2-acceptance.md](milestone-7.2-acceptance.md). |
...
| Atomic manifest bridge install | Yes | Idempotence, staged source failure, complete-set, marker, and source-set hash tests passed | Active manifest and three sources installed | Locked-file platform matrix absent | Automated and smoke verified |
| `game.gd` registration ordering | Yes | Exact contiguous object order tested | `archipelago.o`, then `archipelago-diagnostics.o`, compiled after task control | N/A | Automated and smoke verified |
...
descriptor. The Milestone 7.2 real save/copy/restart matrix passed 12 of 15
mandatory rows; client-only replacement attachment and graceful locked-bank
failure handling remain unresolved, so runtime acceptance is still pending.
...
| Atomic commit and restart | Monotonic revision, clean/unclean restart, stale state, and interrupted-write injection pass. | Game restart recovered the sidecar with a new nonce. Clean and unclean client-only replacement preserved state safely but could not reattach to the still-running official v0.3.5 game, so these mandatory rows failed. |
...
| AP client restart with same game process | Failed for both clean and unclean replacement clients on official v0.3.5: the game retained its descriptor, nonce, receipts, and safe sidecar state, but the replacement compiler/client could not attach. |
