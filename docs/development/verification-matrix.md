...
| Offline location outbox | Task 10 plus nREPL-only task 11 | Atomic local commit, exact partition, offline reopen/rebind, sorted resend, canonical/full and partial/delta reconciliation, rollback-to-pending, confirmation-only compaction, and failure isolation pass | Ordered compile/load only; bound native/server matrix pending | Implemented Milestone 9 slice; no per-send acknowledgement and durable local bits never clear |
