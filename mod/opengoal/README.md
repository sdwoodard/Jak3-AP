...
Jak 3 `data` project. `archipelago.gc` is the game half of protocol 2. It
provides only version/session validation, hello, harmless ping/pong,
disconnect status, framed temporary state export, and protocol logging. It has
no item, location, reward, save, title, mission, or HUD behavior.
...
bridge, and verifies a snapshot-backed hello and ping. The display process
...
file. Stable `[JAK3-AP]` messages identify initialization, hello, ping,
duplicate ping, disconnect, and compatibility results.
